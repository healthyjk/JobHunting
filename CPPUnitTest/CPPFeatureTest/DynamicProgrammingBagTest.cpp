...
/**
  ...
  * 时间：2016年8月11日
  **/
SUITE(DynamicProgrammingBagTest)
{
    class Solution {
    public:
    ...
    TEST_FIXTURE(Solution, NormalTest1)
    {
        int result = dynamicProgrammingBag_normal(vw, vc, V);
        CHECK_EQUAL(27, result);
    }

    TEST_FIXTURE(Solution, ImproveTest1)
    {
        int result = dynamicProgrammingBag_improve(vw, vc, V);
        CHECK_EQUAL(27, result);
    }

    TEST_FIXTURE(Solution, SpecialTest1)
    {
        int result = dynamicProgrammingBag_special(vw, vc, V);
